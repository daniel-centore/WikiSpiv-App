...
export default class Chunk {
    ...
    getReactView (
        ...
        const height =
            (this.text === null ? 0 : LYRICS_HEIGHT) + (!showChord || this.chord === null ? 0 : CHORD_HEIGHT);
        return {
            element,
            height,
        };
...
function ChunkImpl (props: { chord: string | null; text: string | null; lyricIsInstruction: boolean; idx: number }) {
    const dark = useDark();
            ...
            {props.text === null ? null : (
                <Text
                    style={{
                        ...
                        fontWeight: props.lyricIsInstruction ? 'bold' : 'normal',
                        fontStyle: props.lyricIsInstruction ? 'italic' : 'normal',
                        fontSize: 14,
